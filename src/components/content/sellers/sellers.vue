		...
		<div class="seller-wrapper" ref="sellerView">
			...
			<div class="seller-view">
				<div class="address-wrapper">
					<div class="address-left">{{sellers.address}}</div>
					<div class="address-right">
						<div class="content"></div>
					</div>
				</div>
				<div class="pics-wrapper" v-if="sellers.poi_env" ref="picsView">
					<ul class="pics-list" ref="picsList">
						<li ref="picsItem" class="pics-item" v-for="(imgurl,index) in sellers.poi_env.thumbnails_url_list" :key="index">
							<img :src="imgurl" alt="">
						</li>
					</ul>
				</div>
			...
			<div class="tip-wrapper">
				<div class="delivery-wrapper">
					服务配送：{{sellers.app_delivery_tip}}
				</div>
				<div class="shipping-wrapper">
					配送时间：{{sellers.shipping_time}}
				</div>
			</div>
			...
			<div class="other-wrapper">
				<div class="server-wrapper">
					商家服务
					<div class="poi-server" v-for="(item,index) in sellers.poi_service" :key="index">
						<img :src="item.icon" alt="">{{item.content}}
					</div>
				</div>
				<div class="siscounts-wrapper">
					<div class="discounts-item" v-for="(item,index) in sellers.discounts2" :key="index">
						<div class="icon">
		...
		created() {
			...
			.then(response =>{
				if(response.code == 0){
					...
					this.$nextTick(()=>{
						if(this.sellers.poi_env.thumbnails_url_list){
						  let imgW = this.$refs.picsItem[0].clientWidth
						  let marginR = 11
						  let width = (imgW + marginR) * this.sellers.poi_env.thumbnails_url_list.length
